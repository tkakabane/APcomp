...
<strong> ✔️ Note: A single, consolidated script including all steps described above is also provided and reflects the most up-to-date version of the workflow. </strong>
...
To ensure reproducibility, the scripts corresponding to the <em>paleofire</em> functions used in this study are provided here as well (pfAddData | pfCompositeLF | pfTransform). These are also available in the the repository from oblarquez/paleofire, so users should refer to <strong>Blarquez et al. (2014)</strong> for the original methodological framework underlying these functions.
</p>
