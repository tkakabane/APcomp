...
function (ID = NULL, add = NULL, Interpolate = FALSE, Age = NULL, 
  ...
  methods <- c("stl", "Z-Score", "Box-Cox", "LOESS", "MinMax", 
               "RunMed", "RunMean", "RunMin", "RunMax", "RunQuantile", 
  ...
  if (method != "NULL") {
    ...
    for (j in 1:length(method)) {
      ...
      for (k in 1:length(ID)) {
        ...
        if (length(tmp[, 1]) > 3 & ID[k] != 882) {
          ...
          if (methodj == "Z-Score") {
            ...
            if (is.na(mu) | is.na(sigma) | sigma == 0) {
              ...
              transI[, k] <- approx(tmp[, 1], (tmp[, 
                                                   2] - mu)/sigma, Ages[, k])$y
            }
          }
          if (methodj == "Box-Cox") {
            transI[, k] <- approx(tmp[, 1], pfBoxCox(tmp[, 
                                                         2], alpha = alpha, type = type), Ages[, 
                                                                                               k])$y
          }
          if (methodj == "LOESS") {
        ...
        Sys.sleep(2e-05)
        if (k %in% seq(0, length(ID) * length(method), 
                       1) & verbose == TRUE) {
          if (length(method) > 1) {
